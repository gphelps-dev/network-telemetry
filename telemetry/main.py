...
print("Network Telemetry Service Starting")
...
DESTINATIONS = os.getenv("NETWORK_DESTINATIONS", "google.com,github.com,stackoverflow.com").split(",")
...
def collect_telemetry_for_destination(destination):
    ...
    if latency is not None and packet_loss is not None and hops is not None:
        if 0 <= latency <= 10000 and 0 <= packet_loss <= 100 and 0 <= hops <= 50:
            ...
            print("Invalid data for {}: latency={}, packet_loss={}, hops={}".format(destination, latency, packet_loss, hops))
            write_to_influx(destination, latency or 0, packet_loss or 0, hops or 0, success=False)
            return False
    else:
        print("Skipping write for {} due to missing data".format(destination))
        write_to_influx(destination, 0, 0, 0, success=False)
        return False

if __name__ == "__main__":
    print("Network Telemetry Service Started")
